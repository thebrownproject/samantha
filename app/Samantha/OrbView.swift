...
struct OrbView: View {
    ...
    private func gradientColors(state: AppState, time: Double) -> [Color] {
        ...
        switch state {
        case .thinking:
            // Shift hue slightly over time for a shimmering effect
    ...
    private func coreScale(state: AppState, time: Double) -> CGFloat {
        ...
        case .listening:
            // Active breathing: slightly faster, more pronounced
            return 0.74 + sin(time * 2.0) * 0.04
        case .thinking:
            // Steady with micro-pulse
            return 0.73 + sin(time * 3.0) * 0.015
        case .speaking:
            // Rhythmic multi-frequency pulse
    ...
    private func gradientCenter(state: AppState, time: Double) -> UnitPoint {
        switch state {
        case .thinking:
            // Rotating highlight gives "processing" feel
